...
{	
	///
	if (x >= int(Graphics::ScreenWidth) || y >= int(Graphics::ScreenHeight)) return;
	///
	assert( x >= 0 );
...
///
void Graphics::drawRect(int x, int y, int x_length, int y_length, int r, int g, int b)
{
	for (int ii = 0; ii < x_length; ii++)
	{
		for (int jj = 0; jj < y_length; jj++)
		{
			PutPixel(x + ii, y + jj, r, g, b);
		}
	}
}
...
void Graphics::DrawMyCircle(int x, int y, int radius, Color c)
{
	const int rad_sq = radius * radius;
